...
def make_translator(opt, report_score=True, out_file=None):
    ...
    dummy_parser = argparse.ArgumentParser(description='main.py')
    tools.opts.model_opts(dummy_parser)
    dummy_opt = dummy_parser.parse_known_args([])[0]

    fields, model, model_opt = \
        tools.ModelConstructor.load_test_model(opt, dummy_opt.__dict__)
    model.k = opt.k
    scorer = tools.translate.GNMTGlobalScorer(opt.alpha,
                                              opt.beta,
...
class Translator(object):
    ...
    def translate(self, src_dir, src_path, tgt_path,
        ...
        data = tools.io.build_dataset(self.fields,
                                      self.data_type,
                                      src_path,
                                      tgt_path,
                                      src_dir=src_dir,
        ...
        data_iter = tools.io.OrderedIterator(
            dataset=data, device='cuda' if self.gpu>-1 else 'cpu',
            batch_size=batch_size, train=False, sort=False,
            sort_within_batch=True, shuffle=False)

        builder = tools.translate.TranslationBuilder(
            data, self.fields,
    ...
    def translate_batch(self, batch, data):
        ...
        beam = [tools.translate.Beam(beam_size, n_best=self.n_best,
                                     cuda=self.cuda,
                                     global_scorer=self.global_scorer,
                                     pad=vocab.stoi[tools.io.PAD_WORD],
                                     eos=vocab.stoi[tools.io.EOS_WORD],
                                     bos=vocab.stoi[tools.io.BOS_WORD],
        ...
        src = tools.io.make_features(batch, 'src', data_type)
        src_lengths = None
        if data_type == 'text':
        ...
        for i in range(self.max_length):
            ...
            # Run one step.
            if isinstance(self.model, tools.ViModels.ViNMTModel):
    ...
    def _run_target(self, batch, data):
        ...
        if data_type == 'text':
            ...
            src_lengths = None
        src = tools.io.make_features(batch, 'src', data_type)
        tgt_in = tools.io.make_features(batch, 'tgt')[:-1]
        ...
        tgt_pad = self.fields["tgt"].vocab.stoi[tools.io.PAD_WORD]
